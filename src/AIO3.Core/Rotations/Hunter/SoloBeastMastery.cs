...
namespace AIO3.Core.Rotations.Hunter
{
    ...
    public sealed class SoloBeastMastery : IRotation
    {
        ...
        public IReadOnlyList<RotationStep> BuildSteps() => Racials.With(HunterCommon.WithPetSpecials(_settings, new List<RotationStep>
        {
            ...
            // Feed an unhappy pet out of combat (Feed Pet — keeps the pet's damage / loyalty up). Gated on Manage
            // pet too, so the product-owns-the-pet toggle silences it like the rest of the pet upkeep.
            ...
            HunterCommon.MultiShot(_settings, priority: 9f),
