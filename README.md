...
- Settings are typed (`ToggleSetting`, `IntSetting`, `ChoiceSetting`) and exposed by each rotation; values are saved per character under `<WRobot>\Settings\AIO3\<Character>.conf`.
- The settings UI is a **native over-the-game overlay** (a transparent WPF window that tracks the WoW client in borderless / windowed mode) — auto-generated from the typed setting list and bound **directly** to the setting objects, so edits apply live with no Lua bridge. It follows the WoW window (DPI-aware), **remembers its position + state** per character, has a **filter box**, tabbed hover-highlighted rows with **per-setting tooltips** and a per-tab **reset**, and shows a **live status line** (spec · target HP% → current cast). It is **minimizable in-game** to a small status pill that keeps showing that line. See [docs/overlay-design.md](docs/overlay-design.md).
- The overlay deliberately **touches no WoW Lua/UI** — it's a separate window that only reads the WoW window position. The original in-game Lua panel (`SettingsOverlay`, a `CreateFrame` UI driven by an `AIO3Bridge` table) is **retired**: that in-game footprint is observable by a private server's anti-cheat, so it's deactivated (kept unreferenced only as a reference).
